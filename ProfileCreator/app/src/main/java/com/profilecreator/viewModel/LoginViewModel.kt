...
class LoginViewModel(private val loginRepository: LoginRepository) : ViewModel() {
    ...
    val loginResult: LiveData<Response> = _loginResult
    ...
    fun login(username: String, password: String) {
        ...
        if (result is Result.Success) {
            _loginResult.value = Response(
                success = result.data?.email?.let { SuccessMessageView(successMessage = it) }
            )
        } else {
            _loginResult.value = Response(error = R.string.login_failed)
        }
    }
